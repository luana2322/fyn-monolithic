...
public interface MeetupRepository extends JpaRepository<Meetup, UUID> {
    ...
    // Find meetup history for a user (as organizer or participant)
    // Includes: COMPLETED, CANCELLED, EXPIRED meetups
    @Query("""
                SELECT DISTINCT m FROM Meetup m
                LEFT JOIN m.acceptedParticipants p
                WHERE (m.organizer.id = :userId OR p.id = :userId)
                AND m.status IN (
                    com.fyn_monolithic.model.date.MeetupStatus.COMPLETED,
                    com.fyn_monolithic.model.date.MeetupStatus.CANCELLED,
                    com.fyn_monolithic.model.date.MeetupStatus.EXPIRED
                )
                ORDER BY m.scheduledAt DESC
            """)
    Page<Meetup> findMeetupHistory(@Param("userId") UUID userId, Pageable pageable);

    // Find meetups awaiting confirmation for a specific user
    @Query("""
                SELECT DISTINCT m FROM Meetup m
                LEFT JOIN m.acceptedParticipants p
                WHERE (m.organizer.id = :userId OR p.id = :userId)
                AND m.status = com.fyn_monolithic.model.date.MeetupStatus.WAITING_CONFIRMATION
                ORDER BY m.scheduledAt DESC
            """)
    Page<Meetup> findMeetupsAwaitingConfirmation(@Param("userId") UUID userId, Pageable pageable);
}
